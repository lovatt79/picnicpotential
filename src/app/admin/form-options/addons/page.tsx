...
export default function AddonsPage() {
  ...
  async function handleCreate() {
    await supabase.from("form_addon_options").insert(newForm);
    setNewForm({
      label: "",
      price: null,
      price_unit: "flat",
  ...
  async function handleUpdate(id: string) {
    await supabase.from("form_addon_options").update(editForm).eq("id", id);
    setEditingId(null);
    loadItems();
  }
  ...
  async function handleDelete(id: string) {
    ...
    await supabase.from("form_addon_options").delete().eq("id", id);
    loadItems();
  }
  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <h1 className="font-serif text-3xl text-charcoal">Add-on Options</h1>
        <p className="text-warm-gray mt-1">Manage add-on options in the service request form</p>
      </div>
