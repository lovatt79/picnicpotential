...
export default function ColorOptionsPage() {
  ...
  async function handleCreate() {
    await supabase.from("form_color_options").insert(newForm);
  ...
  async function handleUpdate(id: string) {
    await supabase.from("form_color_options").update(editForm).eq("id", id);
    setEditingId(null);
    loadItems();
  }
  ...
  async function handleDelete(id: string) {
    ...
    await supabase.from("form_color_options").delete().eq("id", id);
    loadItems();
  }

  function startEdit(item: ColorOption) {
  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <h1 className="font-serif text-3xl text-charcoal">Color Options</h1>
        <p className="text-warm-gray mt-1">Manage color palette options for picnic setups</p>
      </div>
      ...
      <div className="mb-6">
        ...
        >
          {showNewForm ? "Cancel" : "+ Add New Color Option"}
        </button>
      </div>

      {showNewForm && (
        <div className="bg-white rounded-xl p-6 shadow-sm mb-6">
          <h3 className="font-medium text-charcoal mb-4">New Color Option</h3>
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              ...
              <input
                ...
                placeholder="e.g., Pretty in Pink"
              />
            </div>
            <div>
                ...
                {editingId === item.id ? (
                  <>
                    ...
                    <td className="px-6 py-4 text-right space-x-2">
                      <button onClick={() => handleUpdate(item.id)} className="text-sm text-gold hover:text-gold-dark">
                        Save
                      </button>
                      <button onClick={() => setEditingId(null)} className="text-sm text-warm-gray hover:text-charcoal">
                        Cancel
                      </button>
                    </td>
                  </>
                  ...
                  <>
                    ...
                    <td className="px-6 py-4 text-right space-x-2">
                      <button onClick={() => startEdit(item)} className="text-sm text-gold hover:text-gold-dark">
                        Edit
                      </button>
                      <button onClick={() => handleDelete(item.id)} className="text-sm text-red-600 hover:text-red-800">
                        Delete
                      </button>
                    </td>
                  </>
