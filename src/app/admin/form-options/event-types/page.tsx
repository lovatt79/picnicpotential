...
export default function EventTypesPage() {
  ...
  const [editForm, setEditForm] = useState({ label: "", value: "", sort_order: 0, is_active: true });
  const [showNewForm, setShowNewForm] = useState(false);
  const [newForm, setNewForm] = useState({ label: "", value: "", sort_order: 0, is_active: true });
  ...
  async function handleCreate() {
    await supabase.from("form_event_types").insert(newForm);
    setNewForm({ label: "", value: "", sort_order: 0, is_active: true });
    setShowNewForm(false);
    loadItems();
  }

  async function handleUpdate(id: string) {
    await supabase.from("form_event_types").update(editForm).eq("id", id);
    setEditingId(null);
    loadItems();
  }
  ...
  async function handleDelete(id: string) {
    ...
    await supabase.from("form_event_types").delete().eq("id", id);
    loadItems();
  }

  function startEdit(item: EventType) {
    setEditingId(item.id);
    setEditForm({ label: item.label, value: item.value, sort_order: item.sort_order, is_active: item.is_active });
  }
  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <h1 className="font-serif text-3xl text-charcoal">Event Types</h1>
        <p className="text-warm-gray mt-1">Manage event type options in the service request form</p>
      </div>
      ...
      <div className="mb-6">
        ...
        >
          {showNewForm ? "Cancel" : "+ Add New Event Type"}
        </button>
      </div>
      ...
      {showNewForm && (
        <div className="bg-white rounded-xl p-6 shadow-sm mb-6">
          <h3 className="font-medium text-charcoal mb-4">New Event Type</h3>
          ...
          <div className="mt-4">
            <button
              ...
              disabled={!newForm.label || !newForm.value}
        ...
        <table className="w-full">
          <thead className="bg-sage-light/30 border-b border-sage-light">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium text-charcoal uppercase tracking-wider">Value</th>
              ...
              <tr key={item.id} className="hover:bg-sage-light/10">
                {editingId === item.id ? (
                  ...
                  <>
                    ...
                    <td className="px-6 py-4 text-sm text-warm-gray">{item.value}</td>
