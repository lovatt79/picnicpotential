...
export default function LocationsPage() {
  ...
  const locationTypes = ["winery", "park", "home", "venue", "other"];
  ...
  async function handleCreate() {
    await supabase.from("form_location_options").insert(newForm);
    setNewForm({
      label: "",
      location_type: "winery",
      city: "",
  ...
  async function handleUpdate(id: string) {
    await supabase.from("form_location_options").update(editForm).eq("id", id);
    setEditingId(null);
    loadItems();
  }

  async function handleDelete(id: string) {
    if (!confirm("Delete this location option?")) return;
    await supabase.from("form_location_options").delete().eq("id", id);
    loadItems();
  }
  ...
  function startEdit(item: Location) {
    ...
    setEditForm({
      label: item.label,
      location_type: item.location_type || "winery",
      city: item.city || "",
      notes: item.notes || "",
  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <h1 className="font-serif text-3xl text-charcoal">Location Options</h1>
        <p className="text-warm-gray mt-1">Manage location/venue options for events</p>
      </div>
      ...
      <div className="mb-6">
        ...
        >
          {showNewForm ? "Cancel" : "+ Add New Location"}
        </button>
      </div>
      ...
      {showNewForm && (
        <div className="bg-white rounded-xl p-6 shadow-sm mb-6">
          <h3 className="font-medium text-charcoal mb-4">New Location Option</h3>
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              ...
              <input
                ...
                placeholder="e.g., Kendall Jackson (Santa Rosa)"
              />
            </div>
            <div>
              ...
              <select
                ...
                onChange={(e) => setNewForm({ ...newForm, location_type: e.target.value })}
                className="w-full px-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-gold"
              >
            ...
            <div className="sm:col-span-2">
              ...
              <textarea
                ...
                className="w-full px-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-gold"
                placeholder="e.g., Permission required, outdoor only"
                rows={2}
              />
            </div>
          </div>
        ...
        <table className="w-full">
          <thead className="bg-sage-light/30 border-b border-sage-light">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium text-charcoal uppercase tracking-wider">Type</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-charcoal uppercase tracking-wider">City</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-charcoal uppercase tracking-wider">Notes</th>
              ...
              <tr key={item.id} className="hover:bg-sage-light/10">
                {editingId === item.id ? (
                  ...
                  <>
                    ...
                    <td className="px-6 py-4 text-sm text-warm-gray">{item.city || "—"}</td>
                    <td className="px-6 py-4 text-sm text-warm-gray max-w-xs truncate">
                      {item.notes || "—"}
                    </td>
