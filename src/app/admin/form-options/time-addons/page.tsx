...
export default function TimeAddonsPage() {
  ...
  const [items, setItems] = useState<TimeAddon[]>([]);
  ...
  async function handleCreate() {
    await supabase.from("form_time_addon_options").insert(newForm);
    setNewForm({ label: "", hours: 1, price: 0, sort_order: 0, is_active: true });
    setShowNewForm(false);
    loadItems();
  }

  async function handleUpdate(id: string) {
    await supabase.from("form_time_addon_options").update(editForm).eq("id", id);
    setEditingId(null);
    loadItems();
  }

  async function handleDelete(id: string) {
    if (!confirm("Delete this time add-on?")) return;
    await supabase.from("form_time_addon_options").delete().eq("id", id);
    loadItems();
  }

  function startEdit(item: TimeAddon) {
  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <h1 className="font-serif text-3xl text-charcoal">Additional Time Options</h1>
        <p className="text-warm-gray mt-1">Manage additional time add-ons for events</p>
      </div>
      ...
      <div className="mb-6">
        ...
        >
          {showNewForm ? "Cancel" : "+ Add New Time Add-on"}
        </button>
      </div>
      ...
      {showNewForm && (
        <div className="bg-white rounded-xl p-6 shadow-sm mb-6">
          <h3 className="font-medium text-charcoal mb-4">New Time Add-on</h3>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div>
              <label className="block text-sm font-medium text-charcoal mb-1">Hours *</label>
              <input
                type="number"
                value={newForm.hours}
                onChange={(e) => setNewForm({ ...newForm, hours: parseInt(e.target.value) })}
                className="w-full px-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-gold"
                min="1"
              />
            </div>
          ...
          <div className="mt-4">
            <button
              ...
              disabled={!newForm.label || !newForm.hours}
        ...
        <table className="w-full">
          <thead className="bg-sage-light/30 border-b border-sage-light">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium text-charcoal uppercase tracking-wider">Hours</th>
              ...
              <tr key={item.id} className="hover:bg-sage-light/10">
                {editingId === item.id ? (
                  ...
                  <>
                    ...
                    <td className="px-6 py-4 text-sm text-warm-gray">{item.hours}</td>
                    <td className="px-6 py-4 text-sm text-warm-gray">${item.price.toFixed(2)}</td>
