...
export default function SubmissionDetailPage() {
  ...
  const handleSave = async () => {
    setSaving(true);
    const { error } = await supabase.from("form_submissions").update({ status, admin_notes: adminNotes }).eq("id", params.id);
    setSaving(false);
    if (!error) {
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch("/api/submissions/delete", {
        ...
        body: JSON.stringify({ id: params.id, table: "form_submissions" }),
      });
      if (res.ok) {
            ...
            {isHint ? (
              ...
              <>
                <Section title="Event Details">
                  ...
                  <Field label="Event Time" value={formatTimeStr(submission.event_time)} />
